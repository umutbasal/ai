...
describe("Equality Rules", () => {
  test("where :[x] == :[y] - match equal values", async () => {
    ...
    const result =
      await $`comby 'if (:[x] == :[y])' '' -rule 'where :[x] == :[y]' ${testFile} -match-only`.text();
  ...
  test("compare to literal string", async () => {
    ...
    const result =
      await $`comby 'log(":[level]", ":[msg]")' '' -rule 'where :[level] == "ERROR"' ${testFile} -match-only`.text();
  ...
  test("exclude specific values", async () => {
    ...
    const result =
      await $`comby 'from old_module import :[name]' 'from new_module import :[name]' -rule 'where :[name] != "excluded"' ${testFile} -i`.quiet();
...
describe("Pattern Matching Rules", () => {
  test("match specific pattern", async () => {
    ...
    const result =
      await $`comby 'func_call(":[arg]")' '' -rule 'where match :[arg] { | "error" -> true }' ${testFile} -match-only`.text();
    expect(result).toContain('"error"');
  });
  ...
  test("match test functions using identifier hole with grep", async () => {
    const testFile = setupTestFile("py/sample.py");
    const result =
      await $`comby 'def :[[name]]():' '' ${testFile} -match-only`.text();
    const testFuncs = result
      .split("\n")
...
describe("Duplicate Detection", () => {
  test("find self-assignment", async () => {
    const result =
      await $`echo 'x = x' | comby ':[var] = :[val]' '' -rule 'where :[var] == :[val]' .py -stdin -match-only`.text();
  ...
  test("find duplicate conditions", async () => {
    const testFile = setupTestFile("c/sample.c");
    const result =
      await $`comby 'if (:[x] == :[x])' '' ${testFile} -match-only`.text();
    expect(result).toContain("if (x == x)");
  });

  test("find duplicate boolean checks", async () => {
    const result =
      await $`echo 'if (foo && foo)' | comby 'if (:[x] && :[x])' '' .c -stdin -match-only`.text();
